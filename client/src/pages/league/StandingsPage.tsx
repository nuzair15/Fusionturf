...
export function StandingsPage() {
  ...
  const { data: standings, isLoading, isError, refetch } = useQuery({ queryKey: ["standings-full", seasonId], queryFn: () => api.get<Standing[]>("/league/standings", seasonParams), staleTime: 60000 });
  ...
  return <div className="space-y-8 pb-8">
    <div className="mx-auto max-w-7xl px-4 pt-6 sm:px-6"><Button variant="ghost" onClick={() => navigate("/league")} className="mb-4 gap-1"><ChevronLeft className="h-4 w-4" /> Back to League</Button><SeasonPicker value={seasonId} onChange={setSeasonId} /><LeagueHero eyebrow="Table view" title="League Standings" subtitle={seasonId ? "Season history" : currentSeason?.name || "Current season"} stats={[{ label: "Teams", value: list.length }, { label: "Leader", value: list[0]?.team?.name || "TBD" }, { label: "Points", value: list[0]?.points || 0 }, { label: "Form", value: list[0]?.form || "-----" }]} /></div>
    <div className="mx-auto max-w-7xl px-4 sm:px-6">
      <LeagueCard title="Top two" action={<Badge variant="secondary" className="rounded-full">Final places</Badge>}>
