            ...
            <>
              <div className="hidden overflow-x-auto md:block">
                <table className="w-full text-sm">
                  <thead className="bg-slate-950 text-xs uppercase tracking-wide text-white/75 dark:bg-slate-900">
                    <tr>
                  ...
                  <tbody>
                    {list.map((row) => (
                      <tr key={row.id} className="cursor-pointer border-t transition hover:bg-emerald-500/5" onClick={() => navigate(`/league/teams/${row.team.slug}`)}>
                        <td className="relative p-3 font-semibold"><span className={`absolute inset-y-0 left-0 w-1 ${row.position <= 2 ? "bg-sky-500" : row.position >= list.length - 2 ? "bg-rose-500" : "bg-transparent"}`} />{row.position}</td>
                        <td className="p-3">
