import { useSeasonSelection } from "@/components/league/SeasonPicker";
...
export function TeamDetailPage() {
  const { seasonId, seasonParams } = useSeasonSelection();
  const { slug } = useParams();
  const navigate = useNavigate();
  const { data: team, isLoading, isError, refetch } = useQuery({ queryKey: ["team", slug, seasonId], queryFn: () => api.get<Team>(`/league/teams/${slug}`, seasonParams), enabled: !!slug, staleTime: 0, refetchOnWindowFocus: true, refetchInterval: 30000 });
  ...
  return <div className="mx-auto max-w-7xl space-y-6 px-4 py-6 sm:px-6 sm:py-8"><motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
    ...
    <section className="mt-6"><LeagueCard title="Squad" action={<span className="text-sm text-muted-foreground">{team.players?.length || 0} players</span>}>{team.players?.length ? <div className="grid grid-cols-2 gap-3 p-3 sm:grid-cols-3 sm:p-4 lg:grid-cols-4 xl:grid-cols-5">{team.players.map((player) => <button key={player.id} onClick={() => navigate(`/league/players/${player.slug}?seasonId=${team.seasonId}`)} className="overflow-hidden rounded-2xl border bg-card text-left transition hover:-translate-y-0.5 hover:shadow-md"><div className="aspect-[4/5] bg-muted"><img src={player.photoUrl || "/placeholder.svg"} alt={`${player.firstName} ${player.lastName}`} className="h-full w-full object-cover" /></div><div className="space-y-1 p-3"><p className="truncate text-sm font-bold">{player.firstName} {player.lastName}</p><p className="text-xs text-muted-foreground">{player.position || "Player"}{player.jerseyNumber ? ` · #${player.jerseyNumber}` : ""}</p><p className="text-[11px] font-medium text-primary">Player profile →</p></div></button>)}</div> : <div className="p-4"><LeagueEmptyState title="No squad uploaded" description="Add players to show the squad." /></div>}</LeagueCard></section>
